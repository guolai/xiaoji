...
#import "BBSwipeNavigationController.h"
...
@interface BBSwipeNavigationController ()<UIGestureRecognizerDelegate>
...
@implementation BBSwipeNavigationController
...
- (void)saveSnapshot:(UIImage *)image forViewController:(UIViewController *)controller {
    ...
    if(![[NSFileManager defaultManager] fileExistsAtPath:[BBSwipeNavigationController snapshotCachePath] isDirectory:NULL]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:[BBSwipeNavigationController snapshotCachePath] withIntermediateDirectories:YES attributes:nil error:NULL];
    }
...
- (NSString *)snapshotPathForController:(UIViewController *)controller {
    NSString *snapshotPath = [[BBSwipeNavigationController snapshotCachePath] stringByAppendingFormat:@"/%@.png",NSStringFromClass([controller class]),nil];
    return snapshotPath;
}
